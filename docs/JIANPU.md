...
- 单位仍是 staff space；数字行的**基线 y = 3.0**，数字视觉中心 ≈ y = 2.5
  （数字 size 2.0）。
- 小节线画 y = 1.64…3.0（与数字墨盒同高，不画五条谱线）。
...
**和弦**：`c4+e4+g4` 各成员数字纵向叠放在同一格位，高音在上（自上而下
读），字号缩为主体数字的 0.8 倍，栈以数字行中线（y = 2.32）为中心，
整行墨高随栈高上长。八度点只画最外两侧（最高音的高八度点在栈顶之上、
最低音的低八度点在栈底/减时线之下），内层音省略。各成员的临时记号前缀
共享同一条左挂区、逐音对齐各自数字的中心；自由排布时整栈右移最大外推
量。减时线从栈底（含低八度点）向下排，不穿过和弦；tie/slur 从栈顶起弧。

**装饰音**：`%` 颤音（ornamentTrill）、`$` 短颤音（ornamentShortTrill）、
`&` 波音（ornamentMordent）、`?` 回音（ornamentTurn），SMuFL 字形 0.5
缩放，墨迹水平居中于数字，层叠在音符记号之上（最外侧）；带变音记号的
颤音在其上方再叠小号升降/还原号。

**民乐技法记号**（`TechniqueMark`，可叠加多个）：上滑音 `/`（brassScoop）、
下滑音 `\`（brassFallLipShort）、回滑音 `H`（上滑+下滑并排，组合墨迹整体
居中）、揉弦 `R`（wiggleVibratoWide 波状线）、拨弦 `P`（文字「拨」）、
花舌 `*`（※）、厉音 `L`（⊥）、换气 `V`（∨）、吐音 `T`。装饰滑音按
GB 7.11.7 层叠次序最贴近数字（先于断音/重音记号上叠）；其余技法记号
叠在装饰音之上。仅简谱引擎渲染，五线谱引擎忽略该字段。
...
**减时线分组**与五线谱符杠（规则 7）共享同一实现
`computeBeamRuns`（`layout/beam_grouping.dart`）：拍窗由
`beamGroupBoundaries` 给出（基于 `TimeSignature.beamGroups()`；3/8 型
拍子按 GB/T 46845-2025 §6.3.5.3 整小节一个窗口）。同一拍窗内的八分及
更短音符共享第一条下划线（贯穿组内所有数字底部），第二条下划线只覆盖
十六分音符（类似五线谱的次级符杠分段）。跨拍不连线。组内第一个四分
音符出现时组结束（与符杠组一致）。
...
和弦（纵向叠放）、装饰音（颤音/短颤音/波音/回音）、民乐技法记号
（滑音/回滑/揉弦/拨弦/花舌/厉音/换气/吐音）、多小节结构、与
五线谱一致的间距/确定性约束。

明确不支持（v2+）：小调 6=主音、系统混排对齐、换行、多声部叠写、
简谱 DSL 输入、SMuFL 简谱字形（v1 数字用文本字体）。
